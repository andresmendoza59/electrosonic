...
def plot(filename, metadata, channels):
    ...
    for i, channel in enumerate(channels):
        ...
        points = np.array([timeline, channel]).T.reshape(-1, 1, 2)
        segments = np.concatenate([points[:-1], points[1:]], axis=1)

        norm = plt.Normalize(channel.min(), channel.max())  # Normalize channel data for the colormap
        lc = LineCollection(segments, cmap='viridis', norm=norm)
        lc.set_array(channel)  # Set color based on the channel's amplitude
        lc.set_linewidth(2)

        line = ax[i].add_collection(lc)
        fig.colorbar(line, ax=ax[i], orientation='vertical', label='Amplitude')
